...
public class KurveDriveCommand extends BaseCommand {
	...
	protected void execute() {
		double leftEnc = drivetrainSubsystem.getLeftEncVal();
		double rightEnc = drivetrainSubsystem.getRightEncVal();
		double gyroVal = drivetrainSubsystem.getGyroAngle() * Math.PI / 180;
	...
	protected boolean isFinished() {
		return abs(abs(drivetrainSubsystem.getGyroAngle()) - abs(thetaNeeded)) < Math.PI / 6;
	}
